...
import { fetchUserFiles } from '@/store/thunk';
...
const UserPhotosPage = () => {
  ...
  const { files } = useAppSelector('user');
  const { data: photos, loading, hasMore, pageNo } = files;
  ...
  useEffect(() => {
    if (!loading && photos.length === 0 && hasMore) {
      dispatch(fetchUserFiles({ id: userId, pageNo: 1, pageSize: 10 }));
    }
  }, [photos, loading, userId, hasMore]);

  const handleLoadMore = () => {
    if (!loading && hasMore) {
      dispatch(fetchUserFiles({ id: userId, pageNo, pageSize: 10 }));
    }
  };
  ...
  return (
    ...
    >
      <h1 className='text-2xl font-bold text-white mb-6'>User Photos</h1>
